...
namespace JwtTest.Controllers
{
    public class AccountController : BaseController
    {
        ...
        public AccountController(GuildContext context, IOptions<AuthOptions> options, IHostEnvironment hostEnvironment)
        {
        ...
        {
            User user = context.Users.Find(id);
            if (user != null && !context.Quests.Where(q => q.Client.User.Id == user.Id).Any())
            {
        ...
        public IActionResult Userpage()
        {
            ...
            var adventurer = context.Adventurers.Where(a=>a.User.Id == usr.Id).FirstOrDefault();
            if (adventurer != null)
            {
                usr.Adventurer = new AdventurerModel(){
                    Id = adventurer.Id,
                    Race = usr.Race,
                    AdventurerName = adventurer.AdventurerName,
                    AdventurerRank = adventurer.AdventurerRank,
                    City = usr.City,
                    Country = usr.Country
                };
            }            
        ...
        [Authorize]
        public async Task<IActionResult> DeleteQuest(int IdQuest)
        {
            Quest questDelete = context.Quests.Find(IdQuest);
            if ((questDelete.Client.Id == CurrentUser.Id || CurrentUser.Role != UserRole.User)&&(questDelete.Adventurers.Count == 0)){
                context.Quests.Remove(questDelete);
                await context.SaveChangesAsync();   
            }        
        ...
        [HttpPost]
        public async Task<IActionResult> RegisterQuest(QuestModel model)
        {

            Quest quest = new Quest
            {
                QuestType = model.QuestType,
                QuestRank= model.QuestRank,
                Client = model.Client,
